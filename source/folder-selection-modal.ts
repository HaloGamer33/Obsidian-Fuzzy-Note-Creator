...
export class FolderSelectionModal extends FuzzySuggestModal<Suggestion> {
            ...
            async (event: KeyboardEvent) => {
                ...
                for (let i = 0; i < suggestions.length; i++) {
                    ...
                    this.inputEl.value = suggestions[i]
                        .getText()
                        .replace(' Press ↵ to create folder.', '');
    ...
    getItems(): Suggestion[] {
        ...
        if (
            this.settings.currentFolderRecommendation ||
            this.settings.currentFolderFirst
        ) {
    ...
    onNoSuggestion() {
        ...
        const newDirSuggestion = this.resultContainerEl.createDiv({
            cls: ['suggestion-item', 'is-selected'],
        });

        newDirSuggestion.createEl('span', { text: existingDir });
        newDirSuggestion.createEl('span', {
            cls: 'suggestion-highlight',
            text: newDir,
        });
        newDirSuggestion.createEl('kbd', {
